...
if [[ "$1" == "to" ]]; then
    locate "*.sync" |
    while read file;  do [[ ! ${file} == ${root_sync_path}* ]] && rsync_from_home_to_syncpath $(dirname ${file}); done
fi

if [[ "$1" == "from" ]];then
    locate "*.sync" |
    while read file;  do [[ ${file} == ${root_sync_path}* ]] && rsync_from_syncpath_to_home $(dirname ${file}); done
fi
